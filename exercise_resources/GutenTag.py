...
class GenderClassifier:
    ...
    def __init__(self):
        ...
        if self.suffix_count_cutoff != -1:
            for suffix in list(self.suffix_ratio_dict.keys()):
                if sum(self.suffix_ratio_dict[suffix]) < self.suffix_count_cutoff:
                    del self.suffix_ratio_dict[suffix]
        if self.prefix_count_cutoff != -1:
            for prefix in list(self.prefix_ratio_dict.keys()):
...
class GenreClassifier:
    ...
    def classify_genre(self,text,tags):
        ...
        return max(iter(results.items()),key=operator.itemgetter(1))[0]
...
class Tokenizer:
    ...
    def __init__(self):
        ...
        self.sentence_tokenizer = pickle.load(f)
        f.close()
        self.word_tokenizer = regexp.WordPunctTokenizer()


    def fix_quotes(self,raw_text):
        return self.left_single_quote.sub('\\1 ‘ \\2',self.right_single_quote.sub('\\1 ’ \\2',self.left_double_quote.sub('\\1 “ \\2',self.right_double_quote.sub('\\1 ” \\2',raw_text))))


    def fix_brackets(self,raw_text):
        return self.left_bracket.sub('\\1 \\2',self.right_bracket.sub('\\1 \\2',raw_text))
    ...
    def tokenize_span(self,raw_text):
        ...
        for sentence in sentences:
            sentence = self.word_tokenizer.tokenize(sentence.strip().replace("--","—").replace("-", "hYpppHeN"))
            i = 0
            if not sentence:
                continue
            ...
            while i < len(sentence) -2:
                if sentence[i+1] == "'":
                    if sentence[i+2] in self.contractions:
                        ...
                        i+= 1
                        continue
                elif sentence[i+1] == "." and not (sentence[i+2] == "”" or sentence[i+2] == '’'):
                    sentence[i] += "."
                    del sentence[i+1]
...
class Tag:
    ...
    def get_start_tag(self):
        if self.attributes:
            return "<%s %s>" % (self.tag," ".join([key + '="' + str(value).replace("'","APS").replace(":","_") + '"' for key,value in self.attributes.items() if not key.startswith("@")]))
        else:
            return "<%s>" % (self.tag)
...
class StructureTagger:
    ...
    characters = re.compile("actors|characters|persons|dramatis person(ae|æ)|(^|\W)cast($|\W)")
    ...
    def find_chapters_and_parts(self,front,back,feature_dict,start_lines,end_lines,plike_start,plike_end, global_tags):
        ...
        if feature_dict["epilogue"]:
            possible_epilogues = [x for x in feature_dict["epilogue"] if front <= x < back and x/(back-front) > 0.90] 
            if possible_epilogues:
                ...
                closest_div = min([x for x in possible_default_closest_div if x > index])
        ...
        for div_type in div_types:
            ...
            if div_type in feature_dict and len(feature_dict[div_type]) > 1:
                indicies = list(feature_dict[div_type].keys())
                indicies.sort()
        ...
        if feature_dict["prologue"]:
            possible_prologues = [x for x in feature_dict["prologue"] if front <= x < back and x/(back-front) < 0.10] 
    ...
    def get_top_n(self,D,n):
        return set([word for word, pair in sorted(list(D.items()), key=operator.itemgetter(1),reverse=True)[:3]])
...
class NameTagger():
    ...
    def cluster_names(self,name_count_dict,name_mapping):
        ...
        for word in word_name_dict:
            if word in name_count_dict:
                ...
                if satisfies:
                    most_common = max(list(range(len(counts))),key=lambda x: counts[x])
    ...
    def add_name_tags(self,text):
        ...
        name_mapping = {}
        text.tags.sort()
        for j in range(len(text.tags) + 1):
...
class NamedEntityTagger():
    ...
    def __init__(self,options):
        ...
        self.ratio_dicts = pickle.load(f)
        f.close()
        self.options = options
    ...
    def tag_named_entities(self,text):
        ...
        for name in feature_dict:
            ...
            best_class = max(list(range(len(scores))), key=lambda x: scores[x])
...
def check_said(tokens,p, start_index, end_index):
    if (start_index == p.start or (start_index > 0 and tokens[start_index -1] in StructureTagger.sent_punct)) and (end_index == p.end - 1 or tokens[end_index - 1] in StructureTagger.sent_punct or tokens[end_index - 1] in other_punct or (end_index < len(tokens) -1 and tokens[end_index + 1] in other_punct)):
        return True
    elif (start_index == p.start or (start_index > 0 and (tokens[start_index -1] in StructureTagger.sent_punct or tokens[start_index -1] in other_punct))) and (end_index == p.end - 1 or (tokens[end_index - 1] in StructureTagger.sent_punct or tokens[end_index - 1] == "—")):
        return True
...
class SaidTagger():
    ...
    def add_initial_said_tags(self,text):
        if text.tokens.count('‘') > text.tokens.count('“'):
            start_quote = '‘'
            end_quote = '’'
        else:
            start_quote = '“'
...
class CharacterListBuilder:
    ...
    def build_character_list_and_add_refs(self,text,text_tag_index,xmlids,name_mapping):
        ...
        characters = {character:[] for character in list(character_mapping.values())}
...
class LemmatizerSimple:
    def __init__(self):
        ...
        self.lemma_lookup = pickle.load(f)
        f.close()
...
loc_subject_template = '''    <keywords scheme="#lcsh">
**LOC SUBJECT KEYWORD**
    </keywords>
'''

author_info_simple_template = '''      <author>
       <forename>**AUTHOR FIRST NAME**</forename>
       <surname>**AUTHOR LAST NAME**</surname> 
      </author>
'''

author_info_full_template = '''     <person>
      <persName>
       <forename>**AUTHOR FIRST NAME**</forename>
       <surname>**AUTHOR LAST NAME**</surname>
      </persName>
...
character_entry_template = '''     <person xml:id="**CHAR ID**">
...
character_list_template = '''   <particDesc ana="character_list"**TEXT ID**>
    <listPerson>
**CHAR ENTRIES**
    </listPerson>
   </particDesc>
...
def output_header(fout, tag_dict):
        ...
        for i in range(len(tag_dict["Author"])):
            ...
            for pair in author_pairs:
                if pair[1] in tag_dict and tag_dict[pair[1]] and tag_dict[pair[1]][i] != None:
                    curr_string = curr_string.replace(pair[0],str(tag_dict[pair[1]][i]))
    ...
    if "Characters" in tag_dict:
        ...
        for character_list in tag_dict["Characters"]:
            ...
            character_bits = []
            characters = list(character_list.keys())
...
def split_overlapping_tags(text):
    ...
    for i in range(len(text.tokens) + 1):
        ...
        if i in end_indicies:
            ...
            for j in list(start_indicies.keys()):
...
class GutenTag:
    ...
    def __init__(self,options):
        ...
        if self.options["corpus"] == "PGUSA" and not "build" in options:
            ...
            self.info_lookup = pickle.load(f)
            self.collection_list = pickle.load(f)
            self.firstp_list = pickle.load(f)
            f.close()
...
def insert_measures(text):
    measures = list(textual_measures.keys())
    measures.sort()
...
def keep_server_active():
    while True:
        time.sleep(600)
        f = urllib.request.urlopen("http://localhost:" + str(http_port))
        f.read()
